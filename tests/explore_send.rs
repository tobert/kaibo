...
//! channel handle does — so this holds. If a future change ever holds the kernel
//! across an await inside `call`, or makes `RunExplore` non-`Send`, THIS FILE STOPS
//! COMPILING. That failure is the test. No network.
...
fn run_explore_is_a_send_sync_rig_tool() {
    ...
    assert_send_sync::<RunExplore<anthropic::Client>>();
    ...
    assert_is_tool::<RunExplore<anthropic::Client>>();
}
